...
from adk_agent_skills_primitive.agents.orchestrator import (
    run_adk_skills_primitive_orchestrator,
)
...
app = FastAPI(title="ADK Native Skills Primitive Bakeoff")
...
async def chat(request: ChatRequest):
    ...
    final_answer = await run_adk_skills_primitive_orchestrator(
        request.message,
        request.session_id,
    ...
    log_event(
        ...
        request.session_id,
        "response_ready",
        framework="adk_skills_primitive",
    )
    return {
        "run_id": run_id,
