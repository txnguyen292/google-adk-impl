...
from adk_agent.agents.multi_agent.web_search_agent.agent import app as adk_app
...
def run(message: str | None = typer.Argument(None, help="Message to send.")) -> None:
    load_dotenv()
    logger.info("Running ADK web_search_agent locally")
    try:
        output = asyncio.run(_run_local(_read_message(message)))
    except Exception:
        logger.exception("ADK web_search_agent run failed")
        raise typer.Exit(1)
